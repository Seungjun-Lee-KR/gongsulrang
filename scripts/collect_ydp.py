"""
영등포구 업무추진비 공개 수집 (list + fetch + parse)

- 리스트: https://www.ydp.go.kr/www/selectBbsNttList.do?bbsNo=31&key=2814&pageUnit=30&pageIndex={N}
  - table tbody tr. td[0]=번호, td[1]=제목+nttNo, td[2]=부서, td[3]=작성일, td[4]=조회수, td[5]=파일
  - 단일: downloadBbsFileStr.do?bbsNo=31&atchmnflStr=...
  - 다중: 상세 진입해 downloadBbsFile.do?atchmnflNo=...
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/ydp_posts.json"
PDF_DIR = f"{OUT_DIR}/ydp_pdfs"
RAW_OUT = f"{OUT_DIR}/ydp_expense_raw.csv"

BASE = "https://www.ydp.go.kr"
LIST_URL = f"{BASE}/www/selectBbsNttList.do"
VIEW_URL = f"{BASE}/www/selectBbsNttView.do"
DOWNLOAD_SINGLE = f"{BASE}/www/downloadBbsFileStr.do"
DOWNLOAD_MULTI = f"{BASE}/www/downloadBbsFile.do"
LIST_PARAMS = {"bbsNo": "31", "key": "2814"}
...
def parse_list_row(tr) -> dict | None:
    ...
    for a in tds[-1].find_all("a", href=True):
        fm = re.search(r"atchmnflStr=([A-Za-z0-9_]+)", a["href"])
        if fm:
            files.append({"mode": "single", "str": fm.group(1)})
    if not files and "다중파일" in tds[-1].get_text(" ", strip=True):
        multi = True
    return {"id": ntt_id, "title": title, "dept": dept, "date": date_str,
            "files": files, "multi": multi}
...
def stage_fetch(session: requests.Session):
    ...
    todo = [p for p in posts if p.get("files") or p.get("multi")]
    ...
    for i, p in enumerate(todo, 1):
        ...
        if p.get("multi") and not files:
            try:
                ...
                print(f"  [{i}/{len(todo)}] id={p['id']} multi err={e}")
                n_err += 1
                rate.wait()
                continue
            rate.wait()
        for sn, f in enumerate(files, 1):
            ...
            for ext in ("pdf", "xlsx", "xls"):
                ...
                if os.path.exists(px) and os.path.getsize(px) > 1000:
                    ok = True
                    break
            if ok:
                n_skip += 1
            ...
            try:
                if f["mode"] == "single":
                    r = session.get(DOWNLOAD_SINGLE,
                                    params=dict(LIST_PARAMS, atchmnflStr=f["str"]),
