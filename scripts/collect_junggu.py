...
- 리스트 팝업: https://www.junggu.seoul.kr/popup/open.jsp?custom1=849&page2={N}
  (custom1=849 → 부서별 업무추진비 사용내역)
  - 15 rows/page
  - td[0]=번호, td[1]=제목, td[2]=공표부서, td[3]=공표일, td[4]=조회수
  - view URL: `?custom1=849&mode=view&cid=<cid>`
- 파일 다운로드: /cwsboard/board.do?mode=download&bid=179&cid=<cid>&fileIndex=1&filename=<filename>
  - 상세(view)페이지 진입해서 filename 추출
"""
...
from collect_common import (
    ...
    record_to_csv, load_json, save_json,
)

OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/junggu_posts.json"
CACHE = f"{OUT_DIR}/.junggu_cache.json"
PDF_DIR = f"{OUT_DIR}/junggu_pdfs"
RAW_OUT = f"{OUT_DIR}/junggu_expense_raw.csv"

BASE = "https://www.junggu.seoul.kr"
POPUP = f"{BASE}/popup/open.jsp"
DOWNLOAD = f"{BASE}/cwsboard/board.do"
CUSTOM1 = "849"   # 부서별 업무추진비
BID = "179"
...
_FILE_RE = re.compile(r"/cwsboard/board\.do\?mode=download&(?:amp;)?bid=(\d+)&(?:amp;)?cid=(\d+)&(?:amp;)?fileIndex=(\d+)&(?:amp;)?filename=([^\"'<>\s]+)")
...
def parse_list_row(tr) -> dict | None:
    ...
    return {"id": cid, "num": num, "title": title, "dept": dept, "date": date_str}


def fetch_list_page(session: requests.Session, page: int) -> list[dict]:
    params = {"custom1": CUSTOM1, "page2": str(page)}
    r = session.get(POPUP, params=params, headers=HEADERS, timeout=20)
    r.raise_for_status()
...
def fetch_detail_file(session: requests.Session, cid: str) -> tuple[str, str, str]:
    params = {"custom1": CUSTOM1, "mode": "view", "cid": cid}
    r = session.get(POPUP, params=params, headers=HEADERS, timeout=20)
    r.raise_for_status()
    m = _FILE_RE.search(r.text)
    if not m:
        return "", "", ""
    return m.group(1), m.group(3), m.group(4)  # bid, fileIndex, filename
...
def stage_list(session: requests.Session, since_dt: datetime, max_pages: int):
    ...
    for i, pid in enumerate(todo, 1):
        try:
            bid, file_idx, fname = fetch_detail_file(session, pid)
        except Exception as e:
            print(f"  [{i}/{len(todo)}] id={pid} err={e}")
            bid, file_idx, fname = "", "", ""
        all_posts[pid]["bid"] = bid
        all_posts[pid]["fileIndex"] = file_idx
        all_posts[pid]["filename"] = fname
        detail_done[pid] = {"bid": bid, "fileIndex": file_idx, "filename": fname}
        if i % 50 == 0 or i == len(todo):
            print(f"  [{i}/{len(todo)}] id={pid} file={fname[:40]}")
    ...
    n = sum(1 for p in posts if p.get("filename"))
...
def stage_fetch(session: requests.Session):
    ...
    todo = [p for p in posts if p.get("filename")]
    ...
    for i, p in enumerate(todo, 1):
        if _existing_path(p["id"]):
            ...
            continue
        try:
            referer = f"{POPUP}?custom1={CUSTOM1}&mode=view&cid={p['id']}"
            r = session.get(DOWNLOAD,
                            params={"mode": "download",
                                    "bid": p.get("bid") or BID,
                                    "cid": p["id"],
                                    "fileIndex": p.get("fileIndex", "1"),
                                    "filename": p["filename"]},
...
def stage_parse(limit: int = 0):
    posts = load_json(POSTS, []) or []
    targets = [p for p in posts if p.get("filename")]
    if limit > 0:
