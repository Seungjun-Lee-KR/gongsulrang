"""
동작구 업무추진비 및 강사료 공개 수집 (list + fetch + parse)

- 리스트: https://www.dongjak.go.kr/portal/bbs/B0000591/list.do?menuNo=200209&pageIndex={N}&pageUnit=30
- 상세:   .../portal/bbs/B0000591/view.do?nttId={nttId}&menuNo=200209
- 첨부:   .../portal/cmmn/file/fileDown.do?atchFileId={atchFileId}&fileSn={fileSn}  (PDF)

- 출력:
  - output/dongjak_posts.json
  - output/dongjak_pdfs/{nttId}.pdf
  - output/dongjak_expense_raw.csv
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/dongjak_posts.json"
CACHE = f"{OUT_DIR}/.dongjak_cache.json"
PDF_DIR = f"{OUT_DIR}/dongjak_pdfs"
RAW_OUT = f"{OUT_DIR}/dongjak_expense_raw.csv"

BASE = "https://www.dongjak.go.kr"
LIST_URL = f"{BASE}/portal/bbs/B0000591/list.do"
DETAIL_URL = f"{BASE}/portal/bbs/B0000591/view.do"
DOWNLOAD_URL = f"{BASE}/portal/cmmn/file/fileDown.do"
LIST_PARAMS = {"menuNo": "200209", "pageUnit": "30"}
...
def fetch_list_page(session: requests.Session, page: int) -> list[dict]:
    ...
    for tr in soup.select("table tbody tr"):
        a = tr.find("a", href=re.compile(r"nttId=\d+"))
        if not a:
            continue
        ...
        rows.append({"id": ntt_id, "title": title, "dept": dept, "date": date_str})
    return rows


def fetch_detail_info(session: requests.Session, ntt_id: str) -> dict:
    params = dict(LIST_PARAMS, nttId=ntt_id)
    r = session.get(DETAIL_URL, params=params, headers=HEADERS, timeout=20)
    r.raise_for_status()
    # fileDown.do?atchFileId=<hex64>&fileSn=<n>
    m = re.search(r'fileDown\.do\?atchFileId=([A-Fa-f0-9]+)&(?:amp;)?fileSn=(\d+)', r.text)
    atch_id = m.group(1) if m else ""
    file_sn = m.group(2) if m else ""
    fname = ""
    mn = re.search(r'>\s*([^<>"\']+?\.pdf)\s*<', r.text, re.IGNORECASE)
    if mn:
        fname = mn.group(1).strip()
    return {"atch_file_id": atch_id, "file_sn": file_sn, "pdf_name": fname}
    ...
    for i, pid in enumerate(todo, 1):
        try:
            ...
            try:
                ...
                info = {"atch_file_id": "", "file_sn": "", "pdf_name": ""}
        all_posts[pid].update(info)
        detail_done[pid] = all_posts[pid]
        if i % 25 == 0 or i == len(todo):
            print(f"  [{i}/{len(todo)}] id={pid} atch={info.get('atch_file_id','')[:10]}")
