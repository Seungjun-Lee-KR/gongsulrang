...
- 리스트: https://www.geumcheon.go.kr/portal/selectBbsNttList.do?bbsNo=86&key=269&pageUnit=30&pageIndex={N}
- 상세:   /portal/selectBbsNttView.do?bbsNo=86&key=269&nttNo=...
- 파일:   /portal/downloadBbsFileStr.do?atchmnflStr=<url-encoded-token>
"""
...
from collect_common import (
    ...
    parse_place_with_addr,
)

OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/geumcheon_posts.json"
CACHE = f"{OUT_DIR}/.geumcheon_cache.json"
PDF_DIR = f"{OUT_DIR}/geumcheon_pdfs"
RAW_OUT = f"{OUT_DIR}/geumcheon_expense_raw.csv"

BASE = "https://www.geumcheon.go.kr"
LIST_URL = f"{BASE}/portal/selectBbsNttList.do"
VIEW_URL = f"{BASE}/portal/selectBbsNttView.do"
DOWNLOAD_URL = f"{BASE}/portal/downloadBbsFileStr.do"
LIST_PARAMS = {"bbsNo": "86", "key": "269"}
...
def parse_list_row(tr) -> dict | None:
    ...
    # 날짜는 마지막 td
    date_str = tds[-1].get_text(" ", strip=True).replace(".", "-")
...
def fetch_detail_file(session: requests.Session, ntt_id: str) -> str:
    params = dict(LIST_PARAMS, nttNo=ntt_id)
    r = session.get(VIEW_URL, params=params, headers=HEADERS, timeout=20)
    r.raise_for_status()
    m = re.search(r"downloadBbsFileStr\.do\?atchmnflStr=([^\"'&\s<>]+)", r.text)
...
def stage_list(session: requests.Session, since_dt: datetime, max_pages: int):
    ...
    for i, pid in enumerate(todo, 1):
        try:
            tok = fetch_detail_file(session, pid)
        except Exception as e:
            print(f"  [{i}/{len(todo)}] id={pid} err={e}")
            tok = ""
        all_posts[pid]["atch_str"] = tok
        detail_done[pid] = {"atch_str": tok}
        if i % 50 == 0 or i == len(todo):
            print(f"  [{i}/{len(todo)}] id={pid} tok={tok[:16]}")
    ...
    n = sum(1 for p in posts if p.get("atch_str"))
...
def stage_fetch(session: requests.Session):
    ...
    todo = [p for p in posts if p.get("atch_str")]
...
def stage_parse(limit: int = 0):
    posts = load_json(POSTS, []) or []
    targets = [p for p in posts if p.get("atch_str")]
    if limit > 0:
    ...
    with open(RAW_OUT, "w", encoding="utf-8-sig", newline="") as f:
        ...
        for i, p in enumerate(targets, 1):
            ...
            kept = 0
            for rec in recs:
                name, addr = parse_place_with_addr(rec.get("place", ""))
                if name:
                    rec = dict(rec, place=name)
                out = record_to_csv(rec, district=DISTRICT, dept=dept,
                                    source_id=f"geumcheon_{p['id']}", address=addr)
                if out:
                    w.writerow(out)
                    kept += 1
            n_records += kept
