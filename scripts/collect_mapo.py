"""
마포구 업무추진비 공개 수집 (list + fetch + parse)

- 리스트: https://www.mapo.go.kr/site/main/board/expense/list?cp={N}
- 상세:   /site/main/board/expense/{bcSeq}?bcId=expense
- 첨부:   /site/main/file/download/uu/{uuid}  (PDF)

- 출력:
  - output/mapo_posts.json
  - output/mapo_pdfs/{bcSeq}.pdf
  - output/mapo_expense_raw.csv
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/mapo_posts.json"
PDF_DIR = f"{OUT_DIR}/mapo_pdfs"
RAW_OUT = f"{OUT_DIR}/mapo_expense_raw.csv"

BASE = "https://www.mapo.go.kr"
LIST_URL = f"{BASE}/site/main/board/expense/list"
...
_ROW_LINK_RE = re.compile(r"/site/main/board/expense/(\d+)")
_FILE_LINK_RE = re.compile(r"/site/main/file/download/uu/([a-f0-9]{16,})")


def fetch_list_page(session: requests.Session, page: int) -> list[dict]:
    params = {"cp": str(page), "sortOrder": "BA_REGDATE", "sortDirection": "DESC",
              "listType": "list", "bcId": "expense", "baNotice": "false",
              "baCommSelec": "false", "baOpenDay": "false", "baUse": "true"}
    r = session.get(LIST_URL, params=params, headers=HEADERS, timeout=20)
    r.raise_for_status()
    soup = BeautifulSoup(r.text, "html.parser")
    rows = []
    for tr in soup.select("table tbody tr"):
        a_title = tr.find("a", href=_ROW_LINK_RE)
        if not a_title:
            continue
        m = _ROW_LINK_RE.search(a_title["href"])
        if not m:
            continue
        bc_seq = m.group(1)
        ...
        for td in tds:
            ...
            if _DATE_RE.fullmatch(text):
                md = _DATE_RE.fullmatch(text)
                date_str = f"{md.group(1)}-{int(md.group(2)):02d}-{int(md.group(3)):02d}"
            elif any(sfx in text for sfx in ("과", "실", "단", "담당관")) and 2 <= len(text) <= 25 and not dept:
                if not text.endswith("공개") and "업무추진비" not in text:
                    dept = text
            a_file = td.find("a", href=_FILE_LINK_RE)
            if a_file and not uuid:
                fm = _FILE_LINK_RE.search(a_file["href"])
                if fm:
                    uuid = fm.group(1)
        if not dept:
            dept = extract_dept_from_title(title, DISTRICT)
        rows.append({"id": bc_seq, "title": title, "dept": dept,
                     "date": date_str, "uuid": uuid})
    return rows
...
def stage_fetch(session: requests.Session):
    ...
    print(f"[{DISTRICT}] 파일 다운로드: {len(todo)}건 (PDF/XLSX 혼재)")
    ...
    for i, p in enumerate(todo, 1):
        if _existing_path(p["id"]):
            ...
            continue
        try:
            url = f"{BASE}/site/main/file/download/uu/{p['uuid']}"
