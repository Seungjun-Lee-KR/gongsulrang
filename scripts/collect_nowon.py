...
- 리스트: https://www.nowon.kr/www/user/bbs/BD_selectBbsList.do?q_bbsCode=1012&q_rowPerPage=30&q_currPage={N}
  - table tbody tr. td[0]=번호, td[1]=제목+파일링크, td[2]=부서, td[3]=작성일, td[5]=첨부
- 파일:   /component/file/ND_fileDownload.do?q_fileSn=X&q_fileId=UUID
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/nowon_posts.json"
PDF_DIR = f"{OUT_DIR}/nowon_pdfs"
RAW_OUT = f"{OUT_DIR}/nowon_expense_raw.csv"

BASE = "https://www.nowon.kr"
LIST_URL = f"{BASE}/www/user/bbs/BD_selectBbsList.do"
DOWNLOAD_URL = f"{BASE}/component/file/ND_fileDownload.do"
LIST_PARAMS = {"q_bbsCode": "1012", "q_rowPerPage": "30"}
...
_FILE_RE = re.compile(r"q_fileSn=(\d+)&(?:amp;)?q_fileId=([a-f0-9-]+)")
...
def parse_list_row(tr) -> dict | None:
    ...
    # 파일 정보 — title 내 a 또는 마지막 td
    file_sn = file_id = ""
    for a2 in tr.find_all("a", href=_FILE_RE):
        fm = _FILE_RE.search(a2["href"])
        if fm:
            file_sn, file_id = fm.group(1), fm.group(2)
            break
    dept = tds[2].get_text(" ", strip=True)
    date_str = tds[3].get_text(" ", strip=True)
    return {"id": num_text, "title": title, "dept": dept, "date": date_str,
            "file_sn": file_sn, "file_id": file_id}


def fetch_list_page(session: requests.Session, page: int) -> list[dict]:
    params = dict(LIST_PARAMS, q_currPage=str(page))
...
def stage_list(session: requests.Session, since_dt: datetime, max_pages: int):
    ...
    n = sum(1 for p in posts if p.get("file_sn"))
...
def stage_fetch(session: requests.Session):
    ...
    todo = [p for p in posts if p.get("file_sn")]
    ...
    for i, p in enumerate(todo, 1):
        ...
        try:
            r = session.get(DOWNLOAD_URL,
                            params={"q_fileSn": p["file_sn"], "q_fileId": p["file_id"]},
...
def stage_parse(limit: int = 0):
    posts = load_json(POSTS, []) or []
    targets = [p for p in posts if p.get("file_sn")]
    if limit > 0:
