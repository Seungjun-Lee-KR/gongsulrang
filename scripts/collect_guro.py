...
- 리스트: https://www.guro.go.kr/www/selectBbsNttList.do?bbsNo=655&key=1732&pageUnit=30&pageIndex={N}
  - table tbody tr: td[1] 제목+nttNo, td[2] 부서, td[3] 작성일, td[5] downloadBbsFile.do?atchmnflNo=...
- 파일:   /www/downloadBbsFile.do?atchmnflNo=...
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/guro_posts.json"
PDF_DIR = f"{OUT_DIR}/guro_pdfs"
RAW_OUT = f"{OUT_DIR}/guro_expense_raw.csv"

BASE = "https://www.guro.go.kr"
LIST_URL = f"{BASE}/www/selectBbsNttList.do"
DOWNLOAD_URL = f"{BASE}/www/downloadBbsFile.do"
LIST_PARAMS = {"bbsNo": "655", "key": "1732"}
...
def parse_list_row(tr) -> dict | None:
    ...
    atch_no = ""
    # 파일 링크 찾기 (마지막 td 근처)
    for a in tr.find_all("a", href=re.compile(r"atchmnflNo=\d+")):
        fm = re.search(r"atchmnflNo=(\d+)", a["href"])
        if fm:
            atch_no = fm.group(1)
            break
    return {"id": ntt_id, "title": title, "dept": dept,
            "date": date_str, "atch_no": atch_no}
    ...
    for i, p in enumerate(todo, 1):
        ...
        try:
            r = session.get(DOWNLOAD_URL,
                            params=dict(LIST_PARAMS, atchmnflNo=p["atch_no"], nttNo=p["id"]),
