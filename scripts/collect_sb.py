...
- 리스트: https://www.sb.go.kr/www/selectBbsNttList.do?bbsNo=28&key=5923&pageUnit=30&pageIndex={N}
  - td[0] 번호, td[1] 제목+nttNo, td[2] 부서, td[3] 날짜
- 상세:   /www/selectBbsNttView.do?bbsNo=28&key=5923&nttNo=...
- 파일:   /www/downloadBbsFile.do?atchmnflNo=... (detail 진입 필요)
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/sb_posts.json"
CACHE = f"{OUT_DIR}/.sb_cache.json"
PDF_DIR = f"{OUT_DIR}/sb_pdfs"
RAW_OUT = f"{OUT_DIR}/sb_expense_raw.csv"
...
def fetch_detail_file(session: requests.Session, ntt_id: str) -> str:
    ...
    r.raise_for_status()
    m = re.search(r"downloadBbsFile\.do\?atchmnflNo=(\d+)", r.text)
...
def stage_list(session: requests.Session, since_dt: datetime, max_pages: int):
    ...
    for i, pid in enumerate(todo, 1):
        try:
            atch = fetch_detail_file(session, pid)
        except Exception as e:
            print(f"  [{i}/{len(todo)}] id={pid} err={e}")
            atch = ""
        all_posts[pid]["atch_no"] = atch
        detail_done[pid] = {"atch_no": atch}
        if i % 50 == 0 or i == len(todo):
            print(f"  [{i}/{len(todo)}] id={pid} atch={atch}")
    ...
    n = sum(1 for p in posts if p.get("atch_no"))
...
def stage_fetch(session: requests.Session):
    ...
    todo = [p for p in posts if p.get("atch_no")]
...
def stage_parse(limit: int = 0):
    posts = load_json(POSTS, []) or []
    targets = [p for p in posts if p.get("atch_no")]
    if limit > 0:
