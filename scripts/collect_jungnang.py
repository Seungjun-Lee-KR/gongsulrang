"""
중랑구 업무추진비 공개 수집 (list + fetch + parse)

- 리스트: https://www.jungnang.go.kr/portal/bbs/list/B0000143.do?menuNo=200432&pageIndex={N}
  - table tbody tr. td[0] 제목+상세링크, td[1] 부서, td[2] 파일명+FileDown 링크, td[3] 작성일
- 파일:   /portal/cmm/fms/FileDown.do?atchFileId=FILE_...&fileSn=1

- 출력:
  - output/jungnang_posts.json
  - output/jungnang_pdfs/{nttId}.{pdf|xlsx|xls}
  - output/jungnang_expense_raw.csv
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/jungnang_posts.json"
PDF_DIR = f"{OUT_DIR}/jungnang_pdfs"
RAW_OUT = f"{OUT_DIR}/jungnang_expense_raw.csv"

BASE = "https://www.jungnang.go.kr"
LIST_URL = f"{BASE}/portal/bbs/list/B0000143.do"
DOWNLOAD_URL = f"{BASE}/portal/cmm/fms/FileDown.do"
LIST_PARAMS = {"menuNo": "200432"}
...
_VIEW_RE = re.compile(r"view/[A-Z0-9]+/(\d+)")
_FILE_RE = re.compile(r"atchFileId=(FILE_\d+)(?:&(?:amp;)?fileSn=(\d+))?")
...
def parse_list_row(tr) -> dict | None:
    ...
    if len(tds) < 4:
        return None
    a_title = tds[0].find("a", href=True)
    if not a_title:
        return None
    m = _VIEW_RE.search(a_title["href"])
    if not m:
        return None
    ntt_id = m.group(1)
    ...
    atch_id = ""
    file_sn = "1"
    a_file = tds[2].find("a", href=True)
    if a_file:
        fm = _FILE_RE.search(a_file["href"])
        if fm:
            atch_id = fm.group(1)
            file_sn = fm.group(2) or "1"
    date_str = tds[3].get_text(" ", strip=True)
    return {"id": ntt_id, "title": title, "dept": dept,
...
def stage_fetch(session: requests.Session):
    ...
    for i, p in enumerate(todo, 1):
        if _existing_path(p["id"]):
            ...
            continue
        try:
            headers = dict(HEADERS, Referer=f"{LIST_URL}?menuNo=200432")
