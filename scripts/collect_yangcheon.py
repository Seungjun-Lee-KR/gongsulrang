...
- 리스트: https://www.yangcheon.go.kr/site/yangcheon/ex/bbs/List.do?cbIdx=397&pageUnit=30&pageIndex={N}
  - 행 onclick="doBbsFView('397','<bcIdx>',...)", 제목은 <script>wdigm_title('...')
  - td[2]=부서, td[3]=작성일
- 상세:   /site/yangcheon/ex/bbs/View.do?cbIdx=397&bcIdx=...
- 파일:   /common/board/Download.do?bcIdx=X&cbIdx=397&streFileNm=...
"""
...
OUT_DIR = "data/output"
POSTS = f"{OUT_DIR}/yangcheon_posts.json"
CACHE = f"{OUT_DIR}/.yangcheon_cache.json"
PDF_DIR = f"{OUT_DIR}/yangcheon_pdfs"
RAW_OUT = f"{OUT_DIR}/yangcheon_expense_raw.csv"

BASE = "https://www.yangcheon.go.kr"
LIST_URL = f"{BASE}/site/yangcheon/ex/bbs/List.do"
DETAIL_URL = f"{BASE}/site/yangcheon/ex/bbs/View.do"
DOWNLOAD_URL = f"{BASE}/common/board/Download.do"
CB_IDX = "397"
LIST_PARAMS = {"cbIdx": CB_IDX, "pageUnit": "30"}
...
def parse_list_row(tr) -> dict | None:
    ...
    if len(tds) < 4:
        return None
    a = tds[1].find("a", onclick=True) or tds[1].find("a")
    if not a:
        return None
    ...
    if not m:
        return None
    bc_idx = m.group(2)
    # 제목: script 내 wdigm_title('...')
    title = ""
    for sc in tds[1].find_all("script"):
        tm = re.search(r"wdigm_title\('([^']+)'\)", sc.get_text())
        if tm:
    ...
    return {"id": bc_idx, "title": title, "dept": dept, "date": date_str}
...
def fetch_detail_info(session: requests.Session, bc_idx: str) -> dict:
    params = {"cbIdx": CB_IDX, "bcIdx": bc_idx}
...
def stage_fetch(session: requests.Session):
    ...
    for i, p in enumerate(todo, 1):
        if _existing_path(p["id"]):
            ...
            continue
        try:
            params = {"bcIdx": p["id"], "cbIdx": CB_IDX, "streFileNm": p["stre_file"]}
